...
int hostMatch(long *comparisons) {
	...
	// create myvar to be of type long.
        ...
        #pragma omp parallel for num_threads(2) firstprivate(textData,patternData,lastI, patternLength) private(patternElem,dataElem,i) shared(pindex,comparisons) schedule(static) reduction(+ : myvar)
        for(i=0; i <= lastI; i++) {
                patternElem=0;
				dataElem=i;
				
			//	if(dataElem <= patternElem){
                while(textData[dataElem] == patternData[patternElem]) {
					//printf ("In While");
