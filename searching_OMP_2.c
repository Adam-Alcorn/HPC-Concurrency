...
int hostMatch(long *comparisons) {
	...
	// create myvar to be of type long.
        ...
        #pragma omp parallel for num_threads(1) firstprivate(textData,patternData,lastI, patternLength) private(patternElem,dataElem,i) shared(locOfPatterns,patternFoundIndex) schedule(static) reduction(+ : myvar)
        for(i=0; i <= lastI; i++) {
...
void processData()
{
	...
	result = hostMatch(&comparisons);
			printf ("size of locOfPatterns: %d\n",sizeof locOfPatterns);
